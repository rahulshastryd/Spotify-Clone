...
const loadPlaylist = async (endpoint, elementId) => {
  ...
  for (let { name, description, images, id } of items) {
    ...
    playlistItem.className =
      "bg-black-secondary rounded p-4 hover:cursor-pointer hover:bg-light-black"; //border-solid border-2
    playlistItem.id = id;
...
const fillContentForDashboard = () => {
  ...
  for (let [type, id] of playlistMap) {
    innerHTML += `<article class="p-4">
          ...
          <section class="featured-songs grid grid-cols-auto-fill-cards gap-4" id="${id}"></section>
        </article>`;
  }
...
const formatTime = (duration) => {
  ...
  const formattedTime =
    sec == 60 ? min + 1 + "0:00" : min + ":" + (sec < 10 ? "0" : "") + sec;
...
const loadPlaylistTracks = ({ tracks }) => {
  ...
  for (let trackItem of tracks.items.filter(items=>items.track.preview_url)) {
    ...
    playButton.addEventListener("click", (event) =>
      playTrack(event, { image, artistNames, name, previewUrl, id })
    );
...
const fillContentForPlaylist = async (playlistId) => {
  ...
  coverElement.innerHTML = `
      <img class="object-contain h-64 w-64" src="${images[0].url}" alt="" srcset="">
      <section>
        <h2 id="playlist-name" class="text-4xl">${name}</h2>
        <p id="playlist-details">${tracks.items.length} songs for you</p>
      </section>`
  const pageContent = document.querySelector("#page-content");
